...
TEST(Read, WholeFile) {
  ...
  for (int iter = 0; iter < 10; ++iter) {
    ...
    std::ofstream f("/Volumes/apfs_blank/raw.bin", std::ios::binary | std::ios::trunc);
    f << data;
    f.close();
    ...
    // Mount again
    output.clear();
    ASSERT_EQ(exec("diskutil mountDisk " + disk.raw_device(), output), 0) << output;
  }
}

TEST(Read, MultipleSmallOffsets) {
