...
// (see PhysicalDisks.hpp) with apfs::disk and keeping the ones that are
// APFS-formatted. Also rediscovers any .dmg file still being served by a
// mount helper from a previous GUI session (see MountRegistry.hpp) -- .dmg
...
// apfs_mount_helper process (see MountRegistry.hpp) -- itself a port of
